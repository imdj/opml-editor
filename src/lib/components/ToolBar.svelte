...
<div class="sticky top-0 flex flex-row bg-white z-10 w-full rounded-t-xl p-0.5 border-b-2">
    {#if viewState}
        <div class="flex flex-row flex-grow gap-1 overflow-x-auto">
            <button class="flex flex-row gap-2 py-1 px-2 whitespace-nowrap items-center rounded-lg hover:bg-slate-200" onclick={selectAll}>
                <input type="checkbox" checked={all_selected} class="mr-1"/>
                {all_selected? 'Deselect all' : 'Select all'}
            </button>
            <button class="flex flex-row gap-2 py-1 px-2 whitespace-nowrap items-center rounded-lg hover:bg-slate-200" onclick={() => opml.body = removeItems(opml.body, opml.selectedItems)}>
    ...
    {:else}
        <div class="flex flex-row flex-grow gap-1 overflow-x-auto">
            ...
            <button class="flex flex-row gap-2 py-1 px-2 whitespace-nowrap items-center rounded-lg hover:bg-slate-200" onclick={() => opml.body = removeDuplicates(opml.body)}>
